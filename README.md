...
##  Содержание
  ...
  * [**Войти под созданным пользователем**](#войти-в-систему-под-новым-пользователем)
    ...
    * [Драйверы Wayland и GPU](#драйверы-wayland-и-gpu)
    * [Включение репозитория Multilib](#включение-репозитория-multilib-необязательно)
...
sudo pacman -Syu
```
###  Драйверы Wayland и GPU
```
sudo pacman -S wayland xorg-xwayland qt5-wayland glfw-wayland [xf86-video-ваш тип GPU]
```
Пакеты | Описание
--------- | ----------
wayland       | Cовременный и эффективный протокол для отображения графических приложений в Linux.
xorg-xwayland | Для запуска xclients под Wayland.
qt6-wayland   | API Qt для Wayland.
glfw-wayland  | Gакеты разработки приложений с графическим интерфейсом для Wayland.
...
```
sudo pacman -S plasma konsole dolphin ark kate kcalc krunner partitionmanager packagekit-qt6 plasma-nm plasma-pa sddm-kcm ntfs-3g aspell-ru aspell-en ufw openssh feh kwalletmanager
```
Пакеты | Описание
--------- | ----------
...
packagekit-qt6 | Привязки Qt6 для PackageKit.
...
kwalletmanager | Управление сохраненными паролями

> Для отключения постоянного запроса пароля от kwallet при загрузке, необходимо запустить kwalletmanager и задать для него пустой пароль

#### Включить службу фаервола
```
...
```
> Настройка мгновенного подключния устройств Bluetooth (при наличии поддержки)
```
...
```
> Отключение авто приостановки работы Bluetooth (энергопотребленгие)
```
sudo nano /etc/default/grub
Добавить в переменную GRUB_CMDLINE_LINUX_DEFAULT
btusb.enable_autosuspend=n
Пример:
GRUB_CMDLINE_LINUX_DEFAULT="loglevel=3 quiet btusb.enable_autosuspend=n"

Пересоздать конфигурационный файл grub.cfg
...
```
sudo pacman -S chromium openssh qbittorrent audacious screen wget git neofetch plasma-browser-integration telegram-desktop flameshot xf86-input-synaptics fprint
```
#### Установка через yay
> Должен быть установлен [Yay](#установка--yay-)
...
fprint | Поддержка сканера отпечатков пальца
...
#### Установка
```
sudo pacman -S pipewire pipewire-pulse pipewire-alsa pipewire-plugin-libcamera 
```
Пакеты | Описание
--------- | ----------
...
pipewire-plugin-libcamera | Плагин PipeWire spa для доступа к камерам через libcamera.
...
> На некоторых ПК, звуковые устройства могут не определиться, тогда можно попробовать доустановить пакет sof-firmware ```sudo pacman -S sof-firmware```

> Если будут проблмы со звуком после перезагрузки, то доустановить ```alsa-firmware alsa-utils```
```
sudo pacman -S alsa-firmware alsa-utils
```
> Настройка автоматического переключения звука, после подключения новых устройств
```
